...
let rec leaves_of_tree_acc acc = function
  | D.Leaf(l) -> l::acc
  | D.Node(n) ->
      if Dendogram.leaves_nb n.D.t1 >= 
         Dendogram.leaves_nb n.D.t2 then
        let acc' = leaves_of_tree_acc acc n.D.t1 in
        leaves_of_tree_acc acc' n.D.t2
      else
        let acc' = leaves_of_tree_acc acc n.D.t2 in
        leaves_of_tree_acc acc' n.D.t1
...
let rec fill_tree_pos tree =
  ...
  ExtList.List.iteri (fun i leaf -> leaf.D.index <- i) leaves;

  (*Fills the position of the nodes*)
  let rec fill_node node = match node with
    | D.Leaf(l) ->
        float_of_int l.D.index
    | D.Node(n) ->
        let pos1 = fill_node n.D.t1 in
        let pos2 = fill_node n.D.t2 in
        n.D.pos <- (min pos1 pos2) +. (abs_float (pos2 -. pos1)) /. 2.;
        n.D.pos
...
let upgma dmat collec =
  ...
  let nodes = DynArray.init collec.Genotypes.size D.mk_leaf in
  ...
  while (DynArray.length nodes) > 1 do
    ...
    *)
    ...
    let homology = Dendogram.homology !min_diff collec.Genotypes.geno_size in
    let new_node = Dendogram.mk_node node_a node_b homology in
    ...
    for j=0 to size-1 do
      if (j <> !min_a) && (j <> !min_b) then (
        let leaves_a = float_of_int (Dendogram.leaves_nb node_a) in
        let leaves_b = float_of_int (Dendogram.leaves_nb node_b) in
    ...
    DynArray.add nodes (D.Node(new_node))
  done;
