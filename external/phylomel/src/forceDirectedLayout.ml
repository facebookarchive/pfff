...
let do_calc_spring_forces fs bs tree_fig =
  let parents = tree_fig.Phylogram.tree.Tree.parents in
  let dmat = tree_fig.Phylogram.tree.Tree.dist_mat in
...
let do_calc_spring_forces2 fs bs tree_fig =
  let adj_mat = tree_fig.Phylogram.tree.Tree.adj_mat in
  let dmat = tree_fig.Phylogram.tree.Tree.dist_mat in
