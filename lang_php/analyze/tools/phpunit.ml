...
let final_report ?(report_also_pass=false) tr = 
  ...
  let total_pass = tr +> Common.map_filter (fun t ->
    match t.t_status with Pass i | Fail(_,i) -> Some i | _ -> None) +> 
    Common.sum_int in
...
let json_of_test_status =
  ...
  | Pass v1 ->
      let v1 = J.Int v1 in
      let args = [ v1 ] in J.Array ((J.String "Pass")::args)
  | Fail ((v1, v2)) ->
      let v1 = J.Int v1 and v2 = J.Int v2 in
      let args = [ v1; v2 ] in J.Array ((J.String "Fail")::args)
  | Fatal v1 ->
      let v1 = J.String v1 in
      let args = [ v1 ] in J.Array ((J.String "Fatal")::args)
  ...
  in
    ...
    | (Ocaml.VSum (((("Pass" as tag)), sexp_args)) as sexp) ->
        (match sexp_args with
         | [ v1 ] -> let v1 = Ocaml.int_ofv v1 in Pass v1
