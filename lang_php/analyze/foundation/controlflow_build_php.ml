 ...
 *)
  match error with
  | ColonSyntax info ->
      spos info ^ "Dude, don't use the old PHP colon syntax: "
  | DeadCode (info, node_kind) ->
      spos info ^ "Deadcode path detected " ^
        (F.short_string_of_node_kind node_kind)
  | NoEnclosingLoop info ->
      spos info ^ "No enclosing loop found for break or continue"
  | NoMethodBody info ->
      spos info ^ "Can't compute CFG of an abstract method"
  | DynamicBreak info ->
      spos info ^ "Dynamic break/continue are not supported"
