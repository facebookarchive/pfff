...
(* generated by ocamltarzan: ocamltarzan -vof ast_js.ml *)
...
let _current_precision = ref default_precision
...
and vof_paren _of_a (v1, v2, v3) =
  if !_current_precision.token_info then
  let v1 = vof_tok v1
  and v2 = _of_a v2
  and v3 = vof_tok v3
...
and vof_comma_list _of_a xs = 
  if !_current_precision.token_info
  then Ocaml.vof_list (Common2.vof_either _of_a vof_tok) xs
...
let rec vof_expr (v1, v2) =
  if !_current_precision.type_info
  then
