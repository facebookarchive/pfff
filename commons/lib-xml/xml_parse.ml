...
let parse_file f =
	...
	XmlParser.resolve p (fun file -> 
		...
		Dtd_types.check (Dtd.parse_file name)
	);
...
let buffer_pcdata ?(escape_pcdata=true) text =
  let l = String.length text in
  if escape_pcdata then begin
    for p = 0 to l-1 do 
      match text.[p] with
...
let to_string_fmt ?escape_pcdata x =
  let rec loop ?(newl=false) tab = function
	...
	List.iter buffer_attr alist;
	Buffer.add_string tmp ">";
	buffer_pcdata ?escape_pcdata text;
	Buffer.add_string tmp "</";
	Buffer.add_string tmp tag;
	Buffer.add_char tmp '>';
	if newl then Buffer.add_char tmp '\n';
...
XmlParser._raises (fun x p -> 
	...
	(fun x p -> Dtd_types.Parse_error (x,
