...
(** When parsing an Xml document from a file using the {!Xml.parse_file}
 ...
 {!Xml.File_not_found}). *)
val resolve : t -> (string -> Dtd_types.checked) -> unit
...
(* internal usage only... *)
val _raises : (Xml_types.error_msg -> Lexing.lexbuf -> exn) -> (string -> exn) -> (Dtd_types.parse_error_msg -> Lexing.lexbuf -> exn) -> unit
