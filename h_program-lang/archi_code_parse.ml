...
let re_big_or_external_or_gen = Str.regexp
 ".*/\\(big\\|external\\|gen\\|data\\)/"
...
let source_archi_of_filename3 ~root file = 
  ...
  let f = Common.filename_without_leading_path root file in

  match () with
  | _ when base ==~ re_auto_generated && is_auto_generated file -> AutoGenerated
  | _ when f ==~ re_big_or_external_or_gen -> AutoGenerated
  | _ ->
